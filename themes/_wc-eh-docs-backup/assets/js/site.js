...
function padTocLinks() {
  const tocListItem = Array.from(
    document.querySelectorAll("#TableOfContents ul li")
  );
...
function checkCookie() {
  ...
  if (syslink != "" && syslink != null) {
    ...
    if (
      getCookie("syslink") != "" &&
      getCookie("syslink") != null &&
      getCookie("syslink") != "nosystem"
    ) {
...
function openNavElement(elem, pagename) {
  if (elem) {
    if (elem.hasAttribute("data-parent")) {
      document
        .querySelector(`[data-name="${elem.dataset.parent}"]`)
        .setAttribute("aria-expanded", true);
      openNavElement(
        document.querySelector(`[data-name="${elem.dataset.parent}"]`)
      );
    }
    if (pagename) {
      document
        .querySelector(`[data-name="${pagename}"]`)
        .classList.add("active-nav");
    }
    if (document.querySelector(`[id="collapse-${elem.dataset.name}"]`)) {
      document
        .querySelector(`[id="collapse-${elem.dataset.name}"]`)
        .classList.add("show");
    }
...
window.addEventListener("load", function () {
  ...
  let helpful;
  document
    .getElementById("helpfulModalSend")
    .addEventListener("click", function (e) {
      ...
      const url =
        "https://chat.mieweb.com/hooks/630907bfdd4057df8f60499c/F3b4ZXuscNRtbsgpfarRYw24vZqRAF7f7wjM8wquPReRnPrd";
...
window.addEventListener(
  ...
  function (event) {
    ...
    const link = document.querySelectorAll(
      '.slink, a[href ^= "http://system"], a[href ^= "https://system"], a[href = "#"]'
    );
    ...
    function startSearch(term, doNotAddState) {
      ...
      mainInnerContainer.innerHTML =
        '<div class="container d-flex align-items-center text-center justify-content-center p-5 h3"><i class="fa-solid fa-spinner fa-spin me-3"></i> Searching...</div>';
    ...
    function initIndex() {
      ...
      request.addEventListener(
        ...
        function (event) {
          ...
          if (!documents) {
            console.error(
              "Search index could not be downloaded, was it generated?"
            );
            searchDone();
            return;
          }
          ...
          index = lunr(function () {
            ...
            for (let document of documents) {
              if (document.title && document.title.startsWith("Redirect to: "))
                continue;
              this.add(document);
      ...
      try {
        ...
        for (let result of results) {
          ...
          element.querySelector(".summary").textContent =
            truncateToEndOfSentence(doc.content, 70);
    ...
    if (pagename !== "") {
      ...
      const pagenavparent = document.querySelector(
        `[data-name="${pagenavitem.dataset.parent}"]`
      );
    ...
    document.addEventListener("click", function (e) {
      const hasToggle =
        ...
        e.target.parentElement.classList.contains("btn-toggle");
    ...
    if (syslink != "nosystem") {
      ...
      link.forEach((thisLink) => {
        ...
        const options = {
          ...
          trigger: "click",
          content:
            '<form class="sys_form form"><div class="input-group input-group-sm"><input placeholder="Enter system URL" class="form-control input-sm sys_input" style="width:300px;" type="text"/><button class="btn btn-secondary btn-sm sys_submit" type="submit">Submit</button></div><span id="helpBlock" class="form-text" style="font-size:11px;">The page will refresh. If your system URL is valid you will be able to <br> click on system links.</span></form>',
          placement: "top",
          template:
            '<div style="max-width:none;" class="popover" role="tooltip"><div class="popover-arrow"></div><h3 style="margin-bottom:0px;padding-bottom:8px;font-size:14px;" class="popover-header"></h3><div class="popover-body"></div></div>',
        };
