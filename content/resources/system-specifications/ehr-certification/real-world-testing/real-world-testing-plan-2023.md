...
We will have a weekly selenium test that will choose a certain number of random patient CCDAs in specific live systems, and test for the given sections to exist in the documents.
...
WebChart EHR per certification requirements must be able to receive CCDA Documents with valid security tags.
  
### Test Methodology  
  
We will have Selenium tests that run at minimum weekly to test that the software is still able to generate CCDAs with Security Tags.
...
WebChart EHR per certification requirements must be able to receiveCCDA Documents with security tags and properly display them to end users.
