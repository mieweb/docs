...
With the system setting enabled, navigate to the Provider Organization chart that has the patient/employee portal configured on it. In the _Select Modules_ portal of the portal configuration, make sure the My Appointments module is selected and configured with what is needed for workflows.
...
The **Use Checkin** (when enabled/checkmarked) will allow portal users to _Check into the Waiting Room_ station for their existing scheduled appointment when logged into their own portal account within the My Appointments menu tab in their portal.
