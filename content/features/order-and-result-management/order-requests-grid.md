...
To view recent orders (existing orders) that the patient has had created/ordered for them, go to the _Order Requests_ chart tab.

![](./order-requests-grid.assets/e636c2a39f62b5d6222879edf7b4f74f.png)
...
![](./order-requests-grid.assets/e517a716de03948a224380ee5975848d.png)
...
![](./order-requests-grid.assets/7698ffc3d0dd00da92021d38a1e39cf5.png)
...
![](./order-requests-grid.assets/5739c53e428721b27f585664bb9328a5.png)

If you mark to complete a _standing order_, it doesn't remove the standing order.  It creates a completed version of that order.  To remove the standing order, you have to edit it and take out the order frequency, and then complete it.
...
![](./order-requests-grid.assets/381246f7543fcd44ae4ea911c5b2529c.png)

Simply check-mark the box of the order(s) that you want to select, and then at the bottom of the specific grid section you are in, then use the drop-down to select the ‘category' you wish to (in mass) set these order requests as. Then click the button "_Change Order Category"_ and it will render the selected order requests to be the category you selected it to be set as.

You can complete the entire order or mark certain order items as complete.

![](./order-requests-grid.assets/0ae1d09c35c7a6e78077d444f8fb258b.png)

Selecting the _Completed_ category does not complete the order. It just sets the order as the completed category, but keeps the order request in the ‘pending order requests' grid section. You must still go through the complete link to set the order items within the request to completed.

You can view the category a specific order is set to by viewing the category column.

![](./order-requests-grid.assets/50f0c3772a82ee73e1f54d715731321c.png)

You can also complete all (multiple in mass) with a Results encounter using the button _Add Results_. See section _Add Results_ in this help guide.

## Add Results

The system will display a hyperlink Add Results in the options column, if the encounter visit type "Results" is set as active in the {{% system-name %}} system. If the encounter visit type "Results" is inactive, the _Add Results_ functionality is disabled and not visible. There is also an ability to complete & add results to order requests in mass by checkmarking the order request(s) then clicking button _Add Results_ at the bottom of the grid.

![](./order-requests-grid.assets/880762fa83132a963c3e96f8a2dd80ae.png)

This is ability to complete an order request and document the results within a Results encounter. It will also generate/create a new "Results" encounter in the chart if the system setting _Reuse Encounter when Completing Order_ is disabled (value 0) which is the default behavior. The "Results" encounter is a dynamic encounter that has the following sections:
...
![](./order-requests-grid.assets/29cafa34d1d0e4c97183e27f37343df4.png)
...
![](./order-requests-grid.assets/70f80531336b665e2d711a6a8188bd8c.png)
...
![](./order-requests-grid.assets/0a5e8a2eb7b9a430b8037789c1e437c5.png)
...
![](./order-requests-grid.assets/208295d8eaf2bb854c022acc6afeae3a.png)
...
![](./order-requests-grid.assets/3d05087a0081bca890a321a2f3f411ef.png)
...
![](./order-requests-grid.assets/4cbfc46769e80bb91bcc3fca3e19ecb3.png)
...
![](./order-requests-grid.assets/b89b610004d4fc86389099d81c987319.png)
