...
\_\_\_In order to process the data successfully, the following items must be supplied/completed:
...
\_\_\_Communications control characters will be used for all messages sent between MIE's {{% system-name %}} system and the receiving system. The Minimal Lower Level Protocol (MLLP) is used by the TCP/IP connection, as specified by HL7 standard.
