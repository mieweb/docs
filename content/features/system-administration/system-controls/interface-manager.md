...
![](./interface-manager.assets/2e738cca6cb9a01caf12aee8484a69ad.png)
...
When the Export button is selected, the user will be prompted to select what interfaces to export by selecting checkboxes and then pressing Export again on the popup window, which will download a JSON file.

![](./interface-manager.assets/ce2638d068ded07783031d88b691dc0a.png)

When the Import button is selected, the user is prompted to select a JSON file generated by the Export feature from their computer to upload. Once uploaded the configuration for all interfaces will be loaded into the system and re-populate on screen.

## HR Feed Runtime Metrics

For systems on RC202509 and newer, Administrators with access to the Interface Manager may view HR feed processing metrics. The metrics will display the HR feed status (Successful, Processing, or Failed), as well as the HR feed file name, run time duration, as well as feed start and end times.

![](./interface-manager.assets/c5f34dc471deb4a31a089a9200d85781.png)
