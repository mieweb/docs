...
It will upload the document into the patient's chart. It will state a _Successfully Uploaded_ message and should display the document on the screen with the document identification.

_You can then upload & store any type of file into a patient's chart. However, |system| does not support or have the ability to print/fax or even view certain stored file types._
...
Click on the **Reconcile** link to get into the FroozHIE method to reconcile this data into {{% system-name %}} . (System setting to _reconcile_ must be turned on for this link to show).

![](./ccr-document-import.assets/13fdf188d689b1d97828e0f99205bbac.png)

Once you've clicked _reconcile_, a separate window on your screen which is the FroozHIE module. There are instructions at the top to explain the FroozHIE tool and how to bring data over into {{% system-name %}} .
...
The system setting _Reconcile Link_ can be set with a value of 2 which means {{% system-name %}} will show an "Auto" option in the Reconcile method of the document you are wanting to reconcile into {{% system-name %}} .
