...
The OSHA Statistics report is easily downloadable into a CSV format. Simply click the _Generate CSV_ icon (looks like a corner fold paper icon), which will then turn into a _Download CSV_ icon to click (looks like an in-box tray icon) to generate the report's output into a CSV downloadable file. Store to your folder location of choice.
