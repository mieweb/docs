...
The Due Date range finds any due list items that had/have a due date between that range. The report displays results of what tests & procedures (due list items) that were/are "due" within that specific due date range, as well as its current _status_. Additional search result filters can be done by specific health surveillance panel(s), specific order/test name(s), status and/or requirement. Once the results display in the datavis output, further filtering, grouping, graphing, etc can be done.
