...
Users on RC202309 and newer can switch from the legacy E-Sign view to the DataVis format by changing a My Settings user preference of _Use DataVis in E-sign_ to Yes.
...
Select a row or multiple rows of pending documents that need e-signed, then click the _Review Selected E-Sign_ function button below the datavis table. Doing that then opens a pop-up with the document(s) within it to review and sign document(s) individually from the E-sign Review screen.
